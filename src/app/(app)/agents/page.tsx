...
import { DEMO_AGENTS } from '@/data/mockData';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {DEMO_AGENTS.map((agent) => (
          <Card key={agent.id} hoverable className="p-5 space-y-4">
            ...
            <p className="text-xs text-[#94a3b8] leading-relaxed">{agent.description}</p>

            {agent.currentTaskTitle && (
              <div className="p-2.5 rounded bg-[#080b0c] border border-[#1c2529] space-y-1">
                <div className="text-[10px] font-mono text-[#64748b] uppercase">Current Task</div>
                <div className="text-xs text-[#cbd5e1] truncate">{agent.currentTaskTitle}</div>
              </div>
            )}

            <div className="flex items-center justify-between pt-2 border-t border-[#1c2529] text-xs">
              <div className="font-mono text-[11px] text-[#64748b]">
                Tasks: {agent.totalTasksCompleted} • Spend: {formatCurrency(agent.costUsd)}
              </div>
